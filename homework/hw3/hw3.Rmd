...
P-value is less than $\alpha$. So we will reject $H_0$ and the conclusion si that $\beta_4\neq0$, we should not drop $X_4$ in our model.
...
The model I choose is $Y$~$X_{1}+X_{4}$ and the linear regression equation is $\hat{y}=`r round(the.coef[1],4)`+`r round(the.coef[2],4)`X_{1}+`r round(the.coef[3],4)`X_{4}$.
...
It suggests we can reduce the error by `r round(partial.R2*100, 2)`% if we add $X_{2}$, $X_{3}$ in our model with $X_{1}$, $X_{4}$.
...
I would choose $Y$~$X_{4}$ because the smallest BIC always give us the "correctest" model.
