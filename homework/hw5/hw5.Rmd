...
LL0=logLik(model.0)
#pA = length(model.a$coefficients)
pA = length(coef(model.a))
#p0 = length(model.0$coefficients)
p0 = length(coef(model.0))
LR = -2*(LL0 - LLA)
...
The test statistics LR equals 56.371, p-value equals 1.

**(d)**
```{r, echo = FALSE}

```
p-value is large enough for us to fail to reject $H_0$. We can conclude that X2 can be dropped from the model.
